 ...
 * @description Сервіс синтаксичного розбору текстових повідомлень протоколу шлюзу.
 */

/**
 * Розбиває сирий рядок за розділювачем '#' та формує структурований DTO-об'єкт.
 * Підтримує транзакційну перевірку парності для складених запитів (масове оновлення).
 *
 * @param {string} raw - Сирий текстовий запит (наприклад, "U1#U2#PUT#LANG_JAVA#+5").
 * @returns {Object} Розпарсений об'єкт із полями: senderCode, targetUserCode, method, targetField, opParams (або changes).
 * @throws {Error} Якщо запит порожній або містить недостатню кількість полів.
 */
